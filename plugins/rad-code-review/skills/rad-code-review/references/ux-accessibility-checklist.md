...
### 4.3 Dynamic ARIA States

**What to check**: ARIA state attributes (`aria-expanded`, `aria-selected`, `aria-checked`, `aria-pressed`, `aria-hidden`) must reflect actual component state dynamically, not be hardcoded as string literals.

**Why this matters**: When ARIA states are hardcoded as strings (e.g., `aria-expanded="true"`), screen readers announce a permanently expanded/selected/checked state regardless of actual UI state. A dropdown button that always announces "expanded" is worse than having no ARIA at all — it actively misinforms users.

**How to verify from code**:

Grep for hardcoded ARIA state string literals in JSX/TSX/HTML:
```
...
Each match is a probable hardcoded state. Verify whether the value is static or dynamic:

| Pattern | Status | Action |
|---------|--------|--------|
| `aria-expanded="true"` (JSX string literal) | Hardcoded | Flag — should be `aria-expanded={isOpen}` |
| `aria-expanded={isOpen}` (JSX expression) | Dynamic | Correct |
| `aria-expanded="true"` in static HTML (no JS) | May be intentional | Verify — static content that is always expanded is acceptable |
| `aria-hidden="true"` on a decorative icon | Intentional | Correct — decorative elements should be hidden from screen readers |

**Specific patterns to detect**:

1. **Hardcoded `aria-expanded`**: Common on dropdown triggers, accordion headers, collapsible sections, and hamburger menus. The trigger button's `aria-expanded` must toggle with the panel's visibility.

```jsx
// BAD — always announces "expanded"
<button aria-expanded="true" onClick={() => setOpen(!open)}>
  Menu
</button>
...
2. **Hardcoded `aria-selected`**: Common on tabs, list items in selectable lists, and option elements in custom select components.

```jsx
// BAD — every tab announces "selected"
{tabs.map(tab => (
  <button role="tab" aria-selected="true">{tab.label}</button>
))}

// GOOD — only active tab is selected
{tabs.map(tab => (
  <button role="tab" aria-selected={tab.id === activeTab}>{tab.label}</button>
))}
```

3. **Hardcoded `aria-checked`**: Common on custom checkboxes, switches, and toggle buttons.

```jsx
// BAD — always announces "checked"
<div role="checkbox" aria-checked="false" onClick={toggle}>
  Accept terms
</div>

// GOOD
<div role="checkbox" aria-checked={isChecked} onClick={toggle} onKeyDown={handleKey}>
  Accept terms
</div>
```

4. **Hardcoded `aria-pressed`**: Common on toggle buttons (bold, italic, favorite, like).

```jsx
// BAD
<button aria-pressed="false" onClick={() => setFav(!fav)}>Favorite</button>

// GOOD
<button aria-pressed={isFavorite} onClick={() => setFav(!fav)}>Favorite</button>
```

5. **Frozen `aria-hidden`**: Elements that should conditionally hide from screen readers but have a static `aria-hidden="true"` that never toggles. Common pattern: modal backdrop or dropdown panel that should be `aria-hidden` only when closed.

```jsx
// BAD — content always hidden from screen readers, even when visible
<div aria-hidden="true" className={open ? 'visible' : 'hidden'}>
  {/* dropdown content */}
</div>

// GOOD — hidden state matches visibility
<div aria-hidden={!open} className={open ? 'visible' : 'hidden'}>
  {/* dropdown content */}
</div>
```

**Exception**: `aria-hidden="true"` on decorative elements (icons, SVG decorations, separator lines) is intentional and correct — do not flag these.

**Severity**: Moderate. Hardcoded ARIA states are accessibility violations — screen readers announce incorrect state, which is more confusing than no ARIA at all. WCAG 4.1.2 (Name, Role, Value) Level A.

**Common AI mistakes**: Copying `aria-expanded="false"` from a tutorial and never wiring it to state. Using `aria-selected="true"` on all items in a list. Setting `aria-hidden="true"` to "fix" a screen reader issue without understanding what it hides.
