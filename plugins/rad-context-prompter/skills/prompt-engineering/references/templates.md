...
Full template library for Context Prompt Master. Read the relevant template when the user's task type matches. Do not load all templates at once — only the one you need.
...
| [M — Recursive Thinking](#template-m--recursive-thinking) | Complex problems requiring iterative self-refinement |
...
## Template M — Recursive Thinking

*Use for complex problems with multiple steps, trade-offs, or uncertainties where
a single-pass answer is unlikely to be the best answer. Do NOT use for reasoning-native
models (o3, o4-mini, DeepSeek-R1, Qwen3 thinking mode) — they handle iterative
refinement internally.*

**Three variants by length. Choose based on how much of the system prompt you can
dedicate to this instruction.**

### Variant 1 — Full (system prompt or standalone instruction)

```
You are an advanced reasoning engine that uses recursive thinking for complex
problems. Do not jump straight to a final answer if the task has multiple steps,
...
4. Optional branching for hard problems
   When the problem is highly open-ended or ambiguous:
   - Generate 2–4 distinct candidate plans or lines of reasoning.
...
Unless the user explicitly asks to see your internal reasoning, keep the full
scratch work compact and prioritize a clear, direct final answer.
```

### Variant 2 — Medium (user message or embedded directive)

```
When I say "use recursive thinking," apply this procedure:

1. Restate my goal in one sentence, then list the main subproblems you
   need to solve.
2. For the most important subproblem:
   - Produce a step-by-step reasoning trace and a draft solution.
   - Critique that draft, listing at least 3 concrete weaknesses or risks.
   - Produce an improved solution that fixes those issues.
3. Decide if another critique-and-refine cycle would meaningfully improve
   that subproblem. If yes, run one more cycle; if not, move on.
4. Repeat steps 2–3 for the remaining key subproblems.
5. If the problem is very open-ended, generate 2–3 alternative overall
   approaches, briefly evaluate them, and continue the recursive loop
   only on the best one.
6. Give me one clear final answer, and briefly summarize the main
   reasoning that led you there.

You can show the internal steps compactly, but prioritize a readable
final answer.
```

### Variant 3 — Minimal (embed inside a larger system prompt)
...
- Variant 1 when recursive thinking IS the core behavior (dedicated reasoning agent, complex analysis tool)
- Variant 2 when it's user-triggered ("use recursive thinking" as an activation phrase)
- Variant 3 when it's one capability among many and token budget is tight
