...
**What it isn't.** It is **not** an audit. It does not run axe-core, does not test focus behavior at runtime, and does not test with a screen reader. It cannot tell you whether your alt text is meaningful, whether your reading order makes sense, or whether your focus indicator is sufficiently visible — those need a browser, an AT, or a human. Tailwind contrast ratios *are* computed with real sRGB → WCAG math via `check-tailwind-contrast.py` (v2.1), but contrast on arbitrary inline / runtime-computed styles still needs a browser audit. It is also **not** a compliance certifier; no static tool can produce a defensible WCAG 2.2 AA pass/fail verdict, and rad-a11y does not pretend to.
