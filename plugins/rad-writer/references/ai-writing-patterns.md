...
The skills load this file when checking text. The `ai-audit` skill reports which patterns appear and their tier; the `improve` skill suggests fixes that improve the writing whether or not the source was AI.
...
The `text-stats.py` script computes this directly:
```bash
...
The `text-stats.py` script flags paragraphs with ≥2 transitions:
```bash
...
The `text-stats.py` script measures hedging density per 100 words.
...
The `text-stats.py` script reports em dashes per 250 words. Above 4 was a strong signal in 2024; in 2026 it's elevated but not conclusive.
...
These are reliably clichés in any context. The `check-blocklist.py` script catches them as multi-word phrase patterns.
...
## Measurement (use the scripts)

The `text-stats.py` script computes these directly. Don't ask the LLM to count em dashes; run the script.
