...
|---|---|
| "Frame as craft improvement, not detection" | **Aligned with consensus.** The plugin's `ai-audit` skill says verbatim: "Never claim to detect AI authorship — report patterns, not verdicts." |
| "Lexical blocklist (50+ words)" | **Partial.** Most are still cliché worth replacing; framing them as "AI tells" is a lagging signal in 2026. The blocklist remains in the plugin as **style guidance** (these are stale words), with caveats that their AI-tell status is degrading. |
...
The rad-writer plugin's `ai-writing-patterns.md` reference is structured to align with this view: durable structural tells first, lexical tells second with explicit "this signal is degrading" notes.
