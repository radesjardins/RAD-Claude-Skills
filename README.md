...
|--------|---------------|-----------|
| [rad-session](plugins/rad-session/) | Three-phase workflow lifecycle with cross-machine continuity: `/init` bootstraps a project (deterministic stack detection, plugin recommendations, CLAUDE.md scaffold, gitignore handling for session files), `/startup` orients each session and **verifies sync with origin before reading any handoff file** (3.3 — fetches, lists incoming commits, prompts to pull; `--auto-pull` / `--no-pull` flag overrides), `/wrapup` captures state with prune protection and auto-commits + prompts to push so PC ↔ GitHub ↔ Laptop stay in sync. PreCompact safety net included. | CLI, Desktop |
...
|--------|--------|--------|-------------|-----------|
| [rad-session](plugins/rad-session/) | 4 | 0 | Three-phase workflow lifecycle with **verify-before-read sync** (3.3), **cross-machine continuity** (3.1), and **slim wrapup** (3.2): `/init` (one-time project bootstrap with deterministic stack detection, gitignore handling for session files, rad-* plugin recommendations), `/startup` (per-session orientation; Phase 0 fetches origin and prompts to pull when behind *before* reading any handoff file — listing incoming commits first; `--auto-pull` / `--no-pull` flag overrides; cross-machine handoff detection via hostname in commit message), `/wrapup` (per-session capture with structured "what NOT to do" field, recency-bounded conversation tagging, mechanical session-log derivation, CLAUDE.md prune-with-diff auto-skipped when unchanged, Phase 6 auto-commit + prompted push with `--push` / `--no-push` / `--quick` overrides), `/add-resource`. Two Python scripts (`detect-stack.py`, `detect-resources.py`). PreCompact safety net. Absorbs the archived rad-stack-guide. | CLI, Desktop |
