...
> **May 2026 — rad-session 5.0 lands; rad-planner 4.0 + canonical doc structure now both shipping (marketplace v1.7.0).** rad-session 5.0 picks up rad-planner 4.0's canonical doc-structure contract end-to-end: `docs/status.md` (evidence-based reality, rad-session-owned) replaces `HANDOFF.md`; `docs/planning/archive/` (per-shipped-milestone journal) replaces `.claude/session-log.md`. The **sectioned-writer rule** is now live: rad-planner writes the operating manual's Constitution sections during `/plan` M6; rad-session writes Operational sections (Commands, Compact Instructions, Claude-specific behavior, `@AGENTS.md` shim) during `/init`. **Agent scope routing**: `/init` asks once (Claude only / Codex only / both) and persists to `.rad/profile`; all skills read the profile and write to `CLAUDE.md` and/or `AGENTS.md` accordingly. `/wrapup` is now a 9-phase workflow: refuses if no work changed (Phase 0), writes status.md from EVIDENCE (Phase 2, not chat synthesis), detects candidate decisions mechanically + soft triggers (Phase 3, mode-aware via `.rad/profile`), runs cross-doc validators (Phase 4, from rad-planner), prunes Operational sections only (Phase 5), detects milestone-shipped state and proposes archive (Phase 6), syncs across machines (Phase 7), silent on success (Phase 8). Four end-to-end fixtures under [`plugins/rad-session/fixtures/`](plugins/rad-session/fixtures/) exercise codex-only / claude-and-codex / non-standard-manual / dev-mode scenarios — all four pass `plan-lint`, `status-validator`, `doc-redundancy`, `doc-contradiction`. **Upgrading from 3.x/4.x:** run [`plugins/rad-session/scripts/migrate-to-v5.py`](plugins/rad-session/scripts/migrate-to-v5.py) before installing 5.0. Prior v1.6.0 release (rad-planner 4.0 + rad-session 4.0 with the v4.0 `CLAUDE-FRAGMENT.md` interop pattern) is superseded. See [`docs/doc-conventions.md`](docs/doc-conventions.md) for the canonical structure.
>
